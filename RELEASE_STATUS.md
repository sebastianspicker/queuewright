...
The August 9 record below is historical evidence for its dated candidate. In
the live checkout, the dependency-free Python self-test, 75 Python tests, the
repository verifier (318 files and 26 JSON documents), the ignore-policy check,
and `git diff --check` pass.

The current ignored `studio-ui/dist` predates the live Pages base-path change
and is not candidate evidence: it uses root-relative `/assets/...` URLs that
fail under a `/queuewright/` project subpath. The live Vite configuration now
selects `/queuewright/` for demo builds, but `studio-ui/node_modules` is absent,
so the static artifact, Vitest, Playwright, and rendered-browser checks were not
reproduced. No CI, Pages deployment, or remote service was invoked.
